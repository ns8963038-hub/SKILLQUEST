...
4. From your laptop, create the tables and load the content (57 levels, 20
   ...
   npm run seed                # ends "Seeded: 20 skills, … 57 levels, 20 lessons … Verified."
...
3. It asks for the secret values. Fill in:
   ...
   | skillquest-api | `ADMIN_USER_IDS` | the team's Supabase user ids (UUIDs), comma-separated — Supabase → Authentication → Users, after each has signed up once |
...
7. Sign in with an `ADMIN_USER_IDS` account → the shield icon → **research
