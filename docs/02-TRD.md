...
| Risk model (§6.3) | Random Forest shipped as a joblib artifact | **The days-since-last-practice rule is live** (7 days watch, 14 days at risk). The logistic regression is kept as the report's experiment (`RISK_SCORER=lr`); the Random Forest lost to both. Its number is a **risk score** (days away ÷ 21, capped at 1), not a probability — the database column is still called `probability`, but the admin view and the CSV export say "risk score". Every onboarded student is scored; one who hasn't practised yet counts from onboarding. | Neither trained model beat the rule (LR: +0.0038 PR-AUC, 95% CI [−0.0008, +0.0092]). Deploying the model that didn't win would be the wrong call. |
