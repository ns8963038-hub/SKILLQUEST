...
| 3 | Mastery | Every submit moved BKT, so one level submitted 3 times read "mastered" | **Run examples** (visible tests, never an attempt; written code still counts as practice for the streak) + **Submit**; only a level's **first** submit updates mastery; untouched starter code is refused |
