 ...
 * skills the student has completed. External requirements (things we don't
 * teach) are handled separately by the caller — they're gaps, but not part of
 * this "tracked-skill coverage" percentage.
 */
export function scoreRole(completedSkills: Set<string>, roleSkills: RoleSkill[]): RoleScore {
  ...
  for (const rs of roleSkills) {
    if (completedSkills.has(rs.skillId)) {
