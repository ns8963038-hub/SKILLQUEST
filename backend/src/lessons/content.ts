...
export interface LessonStep {
  id: string;
  type: 'hook' | 'predict' | 'trace' | 'explain' | 'fill';
  // hook / explain
  title?: string;
  body?: string;
  points?: string[];
...
// How many of the lesson's questions the student got right on their first try.
export function scoreAnswers(answered: Record<string, boolean>, steps: LessonStep[]): { correct: number; total: number } {
  const questions = steps.filter((s) => s.type === 'predict').map((s) => s.id);
